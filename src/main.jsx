...
function App() {
  ...
  const metrics = useMemo(() => ({ 'All tickets': tickets.length, Open: tickets.filter(t => t.status === 'Open').length, 'In Progress': tickets.filter(t => t.status === 'In Progress').length, Closed: tickets.filter(t => t.status === 'Closed').length }), [tickets]);
